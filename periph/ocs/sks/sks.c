...
void sks_do_operation( sks_inst *sks ) {
    size_t keysize;
    if ( ~sks->sks_command & SKS_CMD_ALWAYS_SET )
        return;
    ...
    if ( sks->sks_command & SKS_CMD_PRODUCE ) {
        switch( sks->sks_command & SKS_CMD_SOURCE_MASK ) {
        ...
        sks->sks_keys[sks->sks_slot].key_atr1 |= SKS_ATR1_KEY_VALID;
    } else {
        switch( sks->sks_command & SKS_CMD_TARGET_MASK ) {
            ...
            case SKS_CMD_TARGET_AES:
                ...
                break;
            default:
                log(LOG_ERROR, sks->name, "Bad command: 0x%08x!",  sks->sks_command );
                break;//goto err;
        }
    }
    sks->sks_status &= ~SKS_STS_BUSY;
