...
static void
beint_from_bytes(const uint8_t *buffer, uint32_t *data32) {
  int i;
  for (i = 0; i < 16; i++)
    data32[i] =
...
{
  uint32_t state32[8];
  state32[0] = 0x6a09e667;
  state32[1] = 0xbb67ae85;
  state32[2] = 0x3c6ef372;
  state32[3] = 0xa54ff53a;
  state32[4] = 0x510e527f;
  state32[5] = 0x9b05688c;
  state32[6] = 0x1f83d9ab;
  state32[7] = 0x5be0cd19;
  beint_to_bytes(state, state32);
}
...
sha256_transform_bytes(uint8_t *state, const uint8_t *buffer)
{
  uint32_t data32[16];
  uint32_t state32[8];
  beint_from_bytes(state, state32);
  beint_from_bytes(buffer, data32);
  sha256_transform(state32, data32);
  beint_to_bytes(state, state32);
}
